...
@if (count($items) > 0)
    <details class="group relative">
        <summary class="flex cursor-pointer list-none items-center gap-1 text-sm font-medium text-text-DEFAULT transition-colors hover:text-text-muted [&::-webkit-details-marker]:hidden">
            <span>{{ $label }}</span>
...
@else
    <a href="{{ $href ?? '#' }}" class="text-sm font-medium text-text-DEFAULT transition-colors hover:text-text-muted">
        {{ $label }}
    </a>
@endif
